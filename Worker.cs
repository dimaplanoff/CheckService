                ...
                try
                {
                    ...
                    if (Common.ActualTasks.Length != 0)
                    {
                        ...
                        else
                        {
                            ...
                            //провер€ем когда какие задачи были выполнены из лог файла
                            if (File.Exists(fname))
                            {
                                var syslog = File.ReadAllLines(fname).Where(m => !string.IsNullOrEmpty(m)).ToArray();
                                for (int i = 0; i < syslog.Length - 1; i++)
                                {
                                    if (Regex.IsMatch(syslog[i], @"[a-zA-Zа-€ј-я0-9]+") && Regex.IsMatch(syslog[i + 1], @"\d{1,2}\.\d{1,2}\.\d{4}\ \d{1,2}\:\d{1,2}\:\d{1,2}"))
                        ...
                        {                            
                            foreach (STask task in plannedTasks)
                            {
                                ...
                                //пишем системный лог о работе сервиса дл€ последующего цикла
                                Log.Write(task);
                            }
